...
doc_term_df_norm = data.table( data.frame(doc_term_matrix_norm) )

doc_term_df_norm_join = cbind(review_g[, c('year'), with=F], doc_term_df_norm)
...
attach(doc_term_df_norm_join)
aggdata <-aggregate(doc_term_df_norm_join, by=list(year), FUN=mean, na.rm=TRUE)
detach(doc_term_df_norm_join)
